...
# Function 4: Generate response using gpt-4o-mini
def generate_response(question, retrieved_rows):
    ...
    # Use gpt-4o-mini to generate a response based on the combined context
    response = openai.ChatCompletion.create(
        model="gpt-4o-mini",
        messages=[
            {
...
# function to find and retrieve the top 5 results and generate response using gpt-4o-mini
def query_system(description, df, model="text-embedding-ada-002"):
    """Find the closest ISCO code for a given job description."""
    # First, check if we need to translate
    try:
        lang = detect(description)
        if lang != 'en':
            question = gpt_translate_to_english(description, lang)
    except:
        pass  # If language detection fails, proceed with original text

    # Summarize for better embedding
    question = gpt_summarize_text(description)
    ...
    # Retrieve indices of the top 5 most similar rows
    top_5_indices = np.argsort(similarities)[-5:][::-1]  # Sort, get last 5, and reverse to descending order
    retrieved_rows = df.iloc[top_5_indices]

    response = generate_response(question, retrieved_rows)
